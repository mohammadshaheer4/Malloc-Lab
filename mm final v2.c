...
static const size_t chunksize = (1L << 10);    // requires (chunksize % 16 == 0)
...
void free(void *bp)
{
    ...
    change_alloc_next_block (block, false);
    insert_free_block (block);
    //dbg_printf("Completed free(%p)\n", bp);
    //dbg_ensures (find_next (last_block) == ((block_t *)((char *)(mem_heap_hi ()) - (wsize - 1))));
    return;
}
...
static void remove_block (block_t *block)
{
    ...
    /*if (startPoint[ind] == block)
    {
        if (((block -> d).ptrArr[1]) != NULL)
        {
            startPoint[ind] = (block -> d).ptrArr[1];
        }
        else
        {
            startPoint[ind] = freeListPtr[ind];
        }
    }*/
    return;
}
...
static void defer_coalesce ()
{
    block_t *block;
    block_t *newBlock;
    for (block = heap_listp; (get_size (block) > 0); block = find_next (newBlock))
    {
        if (! get_alloc (block))
        {
            remove_block (block);
...
static block_t *find_fit(size_t asize)
{
    ...
    {
        for (iter = freeListPtr[i]; (iter != NULL);
                             iter = (iter -> d).ptrArr[1])
        {
            if (asize <= get_size(iter))
            {
                //startPoint[i] = ((iter -> d).ptrArr[1] == NULL) ? (freeListPtr[i]) : ((iter -> d).ptrArr[1]);
                return iter;
            }
        }
        /*for (iter = freeListPtr[i]; (iter != startPoint[i]); iter = (iter -> d).ptrArr[1])
        {
            if (asize <= get_size(iter))
            {
                startPoint[i] = ((iter -> d).ptrArr[1] == NULL) ? (freeListPtr[i]) : ((iter -> d).ptrArr[1]);
...
{ 
    ...
    for (int i = 0; i < 11; i ++)
    {
                                ...
                                block = (block -> d).ptrArr[1])
        {
            ...
            if (get_alloc (block))
            {
                dbg_printf ("Block (%p) not free. Error on line number %d.\n", block, lineno);
                return false;
            }
            if ((i == 0) && (get_size (block) > pow (2, i + 6)))
            {
                dbg_printf ("Free blocks (%p) not within segregated list size. Size is (%lu). List ind is (%d). Error on line number %d.\n", block, get_size (block), i, lineno);
                return false;
            }
            ...
            else if (get_size (block) <= pow (2, i + 5))
            {
                dbg_printf ("Free blocks (%p) not within segregated list size. Size is (%lu). List ind is (%d). Error on line number %d.\n", block, get_size(block), i, lineno);
                return false;
            }
