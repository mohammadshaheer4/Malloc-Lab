...
/* Array keeping track of blocks with size >= 16 but size < 32 */
static block_t *smallBlocks[5];
...
void *malloc(size_t size) 
{
    ...
    //(max operator)
    asize = max (16, align (size - wsize) + dsize);
    /*if (size <= wsize)
    {
        asize = 32;
    }
...
place(block_t *block, size_t asize)
{
    ...
    if ((csize - asize) >= min_block_size)
    {
        ...
        if ((csize - asize) >= (2 * min_block_size))
        {
        ...
        else
        {
            for (int i = 0; i < 5; i ++)
            {
                if (smallBlocks[i] == NULL)
                {
                    ...
                    smallBlocks[i] = block_next;
                    return;
                }
            }
            for (int i = 0; i < 5; i ++)
            {
                size_t oldSize = get_size (smallBlocks[i]);
                block_t * newBlock = coalesce (smallBlocks[i]);
                if (get_size(newBlock) > oldSize)
                {
                    insert_free_block (newBlock);
                    write_header(block_next, csize-asize, false, true);
                    change_alloc_next_block (block_next, false);
...
static block_t *find_fit(size_t asize)
{
    if (asize < 32)
    {
        for (int j = 0; j < 5; j ++)
        {
            if ((smallBlocks[j] != NULL) && (asize <= get_size (smallBlocks[j])))
            {
                return smallBlocks[j];
            }
        }
    }
    block_t *iter;
