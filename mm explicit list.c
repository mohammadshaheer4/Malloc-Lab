...
static int find_free_list (block_t *block);
...
static block_t *find_fit(size_t asize)
{
    ...
    for (iter = freeListPtr[j]; (iter != NULL);
...
{ 
    ...
    for (int i = 0; i < 10; i ++)
    {
        for (block = freeListPtr[i]; ((block != NULL) && (((block -> d).ptrArr[1]) != NULL)); 
