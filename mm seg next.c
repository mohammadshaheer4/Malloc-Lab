...
static block_t *freeListPtr[5];
static block_t *startIndex[5];
...
static block_t *find_fit(size_t asize)
{
    ...
    for (int i = ind; i < 5; i ++)
    {
        ...
        {
            dbg_printf("the block val is %p.\n", iter);
            dbg_printf("the prev block iter val is %p.\n", (iter -> d).ptrArr[0]);
            if (!(get_alloc(iter)) && (asize <= get_size(iter)))
            {
                if ((iter -> d).ptrArr[1] != NULL)
                {
                    startIndex[i] = (iter -> d).ptrArr[1];
                }
                else
                {
                    startIndex[i] = freeListPtr[i];
                }
...
{ 
                             ...
                             block = find_next(block))
    {
        ...
        if (! get_alloc (block))
        {
            if (((block != heap_listp) && (! get_alloc (find_prev (block)))) || ! get_alloc (find_next (block)))
            {
