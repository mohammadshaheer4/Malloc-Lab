...
static bool sparse_mode = (SPARSE_MODE==1);  
...
static void run_tests(int num_tracefiles, const char *tracedir,
    ...
    for (i=0; i < num_tracefiles; i++) {
        ...
        if (mm_stats[i].valid) {
            ...
            mm_stats[i].secs = sparse_mode ? 1.0 : fsecs(eval_mm_speed, speed_params);
        }
...
int main(int argc, char **argv)
{
    ...
    if (errors == 0) {
        ...
        if (avg_mm_throughput < MIN_SPEED) {
            ...
            p2 = (avg_mm_throughput - MIN_SPEED) /
                (MAX_SPEED - MIN_SPEED)
                * (1.0 - UTIL_WEIGHT);
        }

        if (avg_mm_throughput < MIN_SPEED_CHECKPOINT) {
            p2_checkpoint = 0.0;
        } else if (avg_mm_throughput > MAX_SPEED_CHECKPOINT) {
            p2_checkpoint = 1.0 - UTIL_WEIGHT_CHECKPOINT;
        } else {
            p2_checkpoint =
                (avg_mm_throughput - MIN_SPEED_CHECKPOINT) /
                (MAX_SPEED_CHECKPOINT - MIN_SPEED_CHECKPOINT)
